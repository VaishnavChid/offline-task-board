...
- **Immediate sync on every edit, not debounced or reachability-checked.** `BoardViewModel.perform()` fires a sync right after any local edit succeeds, with no coalescing window and no `NWPathMonitor` check first. A rapid burst of edits fires that many overlapping sync attempts (some redundant Firestore reads), and an offline attempt runs and fails rather than being skipped — traded for a simpler mental model where no individual edit's sync attempt can be silently dropped or delayed by a debounce window.
