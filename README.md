...
- Pull-to-refresh on the task list. Right now a manual re-sync is only reachable via the "Sync" button in the status bar — `.refreshable` on the List would give the standard iOS gesture as a second, more discoverable path to the same `syncNow()` call.
