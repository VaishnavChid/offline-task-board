...
- **Every local edit fires a sync immediately**, not just on launch or a manual tap — `BoardViewModel.perform()` kicks one off right after any create/update/delete/move/reorder succeeds locally. No reachability check first: a sync attempted while offline just fails harmlessly into the same `.failed` retry path any other sync failure uses, so there's nothing extra to guard against. Not debounced, deliberately — a burst of rapid edits fires one sync per edit rather than coalescing them, trading some redundant Firestore reads for the guarantee that no individual edit's sync attempt gets silently dropped by a coalescing window. This is "sync immediately while online," not true background sync — the app still doesn't sync while suspended (see known limitations).
...
- **No sync while backgrounded/suspended.** Sync runs on launch, on every local edit (immediately, while the app is open — see decisions above), and on manual "Sync" tap; nothing runs once the app is backgrounded. True background sync would need `BGTaskScheduler` and a background modes capability — a separate, larger piece of work than firing sync on each edit.
